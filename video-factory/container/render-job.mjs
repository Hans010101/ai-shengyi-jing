...
async function download(item, index) {
  ...
  const headers = { 'User-Agent': 'Mozilla/5.0 AI-Shengyi-Video-Factory' };
  if (item.origin === 'workers-ai-generated' && process.env.RENDER_FETCH_TOKEN) headers['X-Internal-Token'] = process.env.RENDER_FETCH_TOKEN;
...
try {
  ...
  const isComic = manifest.options?.productionLineId === 'comic-engraving-v1';
  ...
  const diagramBeatIndexes = new Set(isComic ? [] : manifest.quality.minUniqueMedia === 0 ? manifest.script.beats.map((_, index) => index) : [2, 4, 6].filter(index => index < manifest.script.beats.length));
  ...
  captions.forEach((caption, index) => {
    ...
    if (isComic) {
      const firstPhrase = index === 0 || captions[index - 1]?.beatId !== caption.beatId;
      if (firstPhrase) {
        const item = (beat?.mediaIds || []).map(id => mediaById.get(id)).find(Boolean);
        if (!item) throw new Error(`COMIC_BEAT_MEDIA_MISSING:${beat?.id || beatIndex}`);
        const beatCaptions = captions.filter(entry => entry.beatId === caption.beatId);
        const beatDuration = beatCaptions.reduce((sum, entry) => sum + entry.duration + manifest.voice.phrasePauseSeconds, 0);
        const overlap = beatIndex > 0 ? .28 : 0;
        sceneHtml.push(`<div id="comic-shot-${beatIndex}" class="shot comic-scene clip" data-start="${Math.max(0, caption.start - overlap).toFixed(3)}" data-duration="${(beatDuration + overlap).toFixed(3)}" data-track-index="${100 + index}" data-layout-allow-overflow><img src="${safe(item.local)}" alt="${safe(item.caption)}"></div>`);
        usedMediaHashes.add(item.hash); mediaUse.set(item.hash, (mediaUse.get(item.hash) || 0) + 1); previousHash = item.hash;
      }
    } else if (diagramBeatIndexes.has(beatIndex)) {
  ...
  const comicCss = `.comic #root{background:#092f28}.comic .head{height:auto;padding:48px 58px;background:linear-gradient(180deg,rgba(4,25,22,.72),transparent);color:#eee7d8}.comic .head .brand{font-size:20px;letter-spacing:.22em;color:#eee7d8;text-shadow:0 2px 8px #03201c}.comic .head .brand b{color:#f5d545}.comic .head h1{display:none}.comic .chapter{display:none}.comic .shot{inset:0;width:100%;height:100%;border-radius:0;box-shadow:none;background:#092f28}.comic .shot img{filter:saturate(.86) contrast(1.1);transform-origin:center center}.comic .shot:after{content:"";display:block;position:absolute;inset:0;background:linear-gradient(180deg,rgba(4,28,24,.18),transparent 36%,transparent 62%,rgba(3,23,20,.68)),radial-gradient(circle at center,transparent 40%,rgba(1,18,15,.38));mix-blend-mode:multiply}.comic .caption{left:54px;right:54px;bottom:104px;height:auto;min-height:180px;justify-content:center;text-align:center;padding:20px 12px;color:#f6db27;font-family:"Noto Sans CJK SC",sans-serif;font-size:54px;font-weight:900;line-height:1.34;letter-spacing:.01em;-webkit-text-stroke:3px #101a16;paint-order:stroke fill;text-shadow:0 5px 2px rgba(0,0,0,.9),0 0 18px rgba(0,0,0,.7)}.comic .footer{z-index:45;bottom:34px;color:rgba(238,231,216,.74);font-size:16px;text-shadow:0 2px 7px #03201c}.comic .progress{height:7px;background:rgba(238,231,216,.2)}.comic .progress i{background:#f6db27}`;
  ...
  const technicalChecks = {
    ...
    diagrams: isComic ? true : renderedDiagramBeats.size >= Math.min(3, Math.floor(manifest.script.beats.length / 2)),
    routeIsolation: isComic ? accepted.length === manifest.script.beats.length && accepted.every(item => item.origin === 'workers-ai-generated') : true
  };
