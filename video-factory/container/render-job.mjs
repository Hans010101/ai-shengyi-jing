...
function logicDiagram(index, beat, timing) {
  ...
  const center = variant.type === 'orbit' ? '<div class="logic-center"><span>商业系统</span><strong>可复制增长</strong></div>' : '';
  return `<div id="logic-${index}" class="shot logic-board logic-${variant.type} clip" ${timing} data-layout-allow-overflow data-diagram-type="${variant.type}"><div class="logic-grid"></div><div class="logic-eyebrow">${variant.eyebrow}</div><h2>${safe(beat?.onScreen || beat?.chapter)}</h2>${paths}${center}<div class="logic-nodes">${nodes}</div><div class="logic-note">不是单点爆款，而是一条可以持续运转的商业链路</div></div>`;
}
...
try {
  ...
  const diagramBeatIndexes = new Set([2, 4, 6].filter(index => index < manifest.script.beats.length));
