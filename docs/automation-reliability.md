...
2026-09-26 复核：9 月 12–25 日任务虽成功，但实际新增为零。
根因是 sitemap 只解析 `/businesses/`，漏掉 `/stories/`；新版视频页也没有传统的标题/摘要结构。
现已同时解析两类 URL，并从视频页公开文字稿提取事实；不绕过付费墙。
...
- 只有本次新增且从未发布的失败详情会隔离；历史案例不会因单条失败被移除，全量质量门禁仍保留。
