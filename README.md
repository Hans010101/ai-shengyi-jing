...
批量案例目录可按以下方式重建。默认只使用现有中文结构化事实；增加 `--fetch-media` 后，会以受控并发发现来源页正文图片和公开视频。5 篇人工精修基准稿默认不会被覆盖。媒体数量以来源页实际公开素材为准，目标为 3—5 个，但不使用广告、水印、工具图标、推荐卡片或无关占位图补数。YouTube/Vimeo 视频保存真实标题、观看地址与公开封面，页面使用可点击视频卡片展示。
